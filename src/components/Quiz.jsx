...
// `teacherMode` reveals every answer immediately (preview / answer key for the tutor).
export default function Quiz({ set, onBack, teacherMode = false }) {
  const snapshot = useStore();
  const saved = snapshot.attempts[set.id] || {};

  const [selected, setSelected] = useState({});
  const [sprInputs, setSprInputs] = useState({});
  const [revealed, setRevealed] = useState(() => (teacherMode ? Object.fromEntries(set.problems.map((p) => [p.id, true])) : {}));
  ...
  const handleReveal = (pid) => {
    ...
    if (!teacherMode) recordAttempt(set.id, p, getAnswer(p));
  ...
  const scoreAll = () => {
    ...
    set.problems.forEach((p) => {
      nr[p.id] = true;
      const correct = recordAttempt(set.id, p, getAnswer(p));
      if (correct) n += 1;
    });
    setRevealed(nr);
    setScore(n);
  ...
  const reset = () => {
    setSelected({}); setSprInputs({}); setRevealed({}); setScore(null); setShowKey(false);
    resetSet(set.id);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "16px 16px 60px", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ marginBottom: 28, textAlign: "center" }}>
        <div style={{ display: "inline-block", background: set.accent, color: "#fff", fontSize: 10, fontWeight: 700, letterSpacing: 1.5, padding: "4px 12px", borderRadius: 20, marginBottom: 12, textTransform: "uppercase" }}>{set.label}{teacherMode ? " · 선생님 미리보기" : ""}</div>
          ...
          <div key={p.id} style={{ background: "#fff", borderRadius: 14, marginBottom: 20, padding: "20px 20px 16px", border: isR && !teacherMode ? `1.5px solid ${cor ? "#66BB6A" : "#EF5350"}` : "1px solid #E8E8E8" }}>
            ...
            {isR && (
              <div style={{ background: "#FAFAFA", borderRadius: 10, padding: 16, marginTop: 8 }}>
                {!teacherMode && <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.5, margin: "0 0 10px", color: cor ? "#43A047" : "#E53935" }}>{cor ? "✓ CORRECT" : "✗ INCORRECT"}</p>}
      ...
      {!teacherMode && (
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap", padding: "12px 0 16px" }}>
          {score === null && <button onClick={scoreAll} style={{ background: "#1a1a1a", color: "#fff", border: "none", borderRadius: 10, padding: "12px 28px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Score All ({answeredCount}/{set.problems.length})</button>}
